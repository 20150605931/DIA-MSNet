...
```markup
# Use the GPU to train the DIA-MSNet model
CUDA_VISIBLE_DEVICES=1 python tools/train.py models/yaml/DIA-MSNet.py --device cuda --kflod-validation -1
# Or use the K-fold cross validation to train the DIA-MSNet model
CUDA_VISIBLE_DEVICES=1 python tools/train.py models/yaml/DIA-MSNet.py --device cuda --kflod-validation 0


# Verify the DIA-MSNet model. Note that you need to replace test=dict() in models/yaml/DIA-MSNet.py with the trained weights
python tools/evaluation.py models/yaml/DIA-MSNet.py --kflod-validation -1
# Or use the following command when performing K-fold cross-validation 
python tools/evaluation.py models/yaml/DIA-MSNet.py --kflod-validation 0

# Visual CAM picture
python tools/vis_cam.py ./datasets/test/M models/yaml/DIA-MSNet.py --save-path ./logs/CAM_visualization/M --target-category 1
```
